...
public class NotificationDeadLetterTest {
    ...
    void setUp() {
        ...
        // Pre-configure notification with 5 retries (max default is 5)
    ...
    @Test
    void testNotificationTransitionsToDeadLetterOnExhaustion() {
        retryService.scheduleRetry(notification, new RuntimeException("Final fail"));

        Notification updated = notificationRepository.findById(notification.getId()).orElseThrow();
        assertEquals(NotificationStatus.DEAD_LETTER, updated.getStatus());
        assertEquals("Final fail", updated.getFailureReason());
    }
}
